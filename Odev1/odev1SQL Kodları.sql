...
SELECT zadi as Malzeme FROM 
    malzeme, musteri, siparis, kullan WHERE 
    musteri.madi = 'Ali' AND 
    musteri.msoyadi='Kurt' AND 
    musteri.mno=siparis.mno AND 
    siparis.yno=kullan.yno AND 
    kullan.zno=malzeme.zno;

/* Soru 2 */
SELECT yemek.yno as Yemek_No, yadi as Adi, ykalori as Kalori, yfiyati as Fiyat, yturu as Tur FROM 
    yemek WHERE
    yemek.yno NOT IN (
